...
\section{Pipeline Summary and Discussion}

Figure~\ref{fig:pipeline_summary} provides a comprehensive summary dashboard of the full pipeline run, combining key metrics from query generation, labeling, training, and evaluation.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{analysis_pipeline_summary.png}
\caption{Pipeline summary dashboard aggregating key metrics from all stages. This consolidated view enables rapid assessment of the entire experimental run, from data generation quality through model training convergence to final estimation accuracy.}
\label{fig:pipeline_summary}
\end{figure}

The experimental results demonstrate several important findings. The schema-aware validation pipeline effectively filters invalid LLM-generated queries, with the majority passing all validation layers. The structural feature distributions confirm that the diversity hint rotation mechanism successfully prevents mode collapse during generation, producing a balanced mix of query complexities.

The generated workload covers cardinalities spanning multiple orders of magnitude, which is essential for training a generalizable cardinality estimator. The active learning curves show that model accuracy improves steadily with each round, confirming that intelligent query selection substantially reduces the number of expensive database executions required for training.

The predicted versus actual scatter plot demonstrates good model calibration across the full cardinality range. The training loss curves confirm stable convergence within each round, with each subsequent round benefiting from the expanded training set. The comparison with PostgreSQL's native optimizer provides valuable context for the MSCN model's performance, situating the learned approach relative to the established baseline used in production database systems.
