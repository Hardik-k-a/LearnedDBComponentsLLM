...
One important advancement in the area of learning-based query cardinality estimation is the development of a new technique called Multi-Set Convolutional Network (MSCN), proposed by Kipf et al.~\cite{DBLP:conf/cidr/KipfKRLBK19}. The technique reformulates query cardinality estimation as a supervised learning task, where query result sizes are estimated based on query structures.
...
The ability to quantify the uncertainty of a machine learning model's predictions is crucial for deciding whether a given estimate can be trusted. In the context of neural networks, one of the most effective techniques for uncertainty quantification is Monte Carlo (MC) dropout. To understand MC dropout, it is first necessary to examine the theoretical foundations of standard dropout.

Dropout was originally introduced as a regularization technique to prevent overfitting. As formally analyzed by Baldi and Sadowski~\cite{DBLP:journals/ai/BaldiS14}, dropout forces neurons to become robust and rely on population behavior rather than co-adapting with specific units. Their theoretical framework demonstrates that applying dropout to a neural network essentially performs a geometric averaging over an ensemble of all possible subnetworks. By stochastically dropping units during training, the network is trained as a massive ensemble, and during standard inference, weight scaling approximates this ensemble average.

While standard dropout is turned off during inference, Gal and Ghahramani~\cite{gal2016dropout} demonstrated that keeping dropout active during prediction allows the network to function as a Bayesian approximation. By running multiple forward passes for the same input with different dropout masks, the network produces a distribution of predictions. The variance across these predictions serves as a principled measure of the model's epistemic uncertainty.

However, a major limitation of MC dropout is its computational cost. Running dozens of forward passes for every query during inference introduces significant overhead, which has traditionally limited its use in performance-critical applications. Recent advancements have sought to address this bottleneck. For instance, Srinivasan~\cite{Srinivasan2024Efficient} introduced a model-splitting technique that significantly accelerates MC dropout inference. By recognizing that dropout is often only applied to the final layers (the model head), the deterministic lower layers (the model body) only need to be executed once. The intermediate activations are then passed repeatedly through the stochastic head, yielding a potential 25x to 33x speedup. This makes MC dropout practically viable for rapid, millisecond-level uncertainty quantification, enabling its use in dynamic environments like active learning for database query optimization.

\section{Active Learning for Data Efficiency}

Active learning helps in developing a framework for improving data efficiency, which can be done through the selection of the best data samples for learning. As discussed by Li et al.~\cite{DBLP:journals/tnn/LiWCJDO25}, not all data samples are equally important in improving the performance of the model. This way, it is possible to attain high accuracy with less data.
