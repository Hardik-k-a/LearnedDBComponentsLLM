...
\section{System Perspective: Ollama and Practical Deployment}

In terms of system efficiency, the application of LLMs for data generation also requires careful consideration. In this work, Ollama is used to locally run LLMs. This allows for efficient query generation. Local running also provides several advantages, including efficiency, privacy, and ease of experimentation. 

Another advantage of using local running for LLMs is the ability to control the models. This provides reproducibility of experiments. Additionally, this approach helps to ensure that the results are not affected by external APIs. Local running also provides the advantage of fine-tuning models based on specific requirements.

The application of Ollama provides the advantage of generating queries. This process can be refined and validated. This setup provides the advantage of ensuring that the generated data meets the requirements for the training process.
